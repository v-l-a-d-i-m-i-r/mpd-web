...
try {
  ...
  const jsonRpcRouter = new JSONRPCRouter(router);

  MPDModule.new(router, jsonRpcRouter, logger);

  const server = createServer((req: IncomingMessage, res: ServerResponse): void => router.handle(req, res));
